...
if __name__ == '__main__':
    ...
    train_dataset = l2l.vision.datasets.MiniImagenet(root=path_data, mode='train')
    valid_dataset = l2l.vision.datasets.MiniImagenet(root=path_data, mode='validation')
    test_dataset = l2l.vision.datasets.MiniImagenet(root=path_data, mode='test')

    
    train_sampler = l2l.data.NShotKWayTaskSampler(train_dataset.y, 100, args.train_way, args.shot, args.train_query)
    
    train_loader = DataLoader(dataset=train_dataset, batch_sampler=train_sampler,
                              num_workers=8, pin_memory=True)

    val_sampler = l2l.data.NShotKWayTaskSampler(valid_dataset.y, 400, args.test_way, args.shot, args.train_query)

    val_loader = DataLoader(dataset=valid_dataset, batch_sampler=val_sampler,
                            num_workers=8, pin_memory=True)
    
    test_sampler = l2l.data.NShotKWayTaskSampler(test_dataset.y, 2000, args.test_way, args.test_shot, args.test_query)
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            loss, acc = fast_adapt(batch, args.train_way, args.shot, args.train_query, metric=pairwise_distances_logits)
        ...
        for i, batch in enumerate(val_loader, 1):
            loss, acc = fast_adapt(batch, args.test_way, args.shot, args.train_query, metric=pairwise_distances_logits)
    ...
    for i, batch in enumerate(test_loader, 1):
        _, acc = fast_adapt(batch, args.test_way, args.test_shot, args.test_query, metric=pairwise_distances_logits)
        loss_ctr+=1
        n_acc += acc
